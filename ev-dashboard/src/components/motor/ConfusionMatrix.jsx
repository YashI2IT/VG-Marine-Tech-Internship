...
const SHORT = { Healthy: 'Healthy', Elec_Damage: 'Elec', Mech_Damage: 'Mech', Mech_Elec_Damage: 'M+E' }
...
export default function MotorConfusionMatrix({ meta }) {
  ...
  const maxVal = cm.flat().reduce((a, b) => Math.max(a, b), 0)
                  ...
                  {row.map((val, ci) => {
                    ...
                    return (
                      <td key={ci} className="px-1 py-1 text-center">
                        <div className="w-14 h-10 rounded-lg flex flex-col items-center justify-center mx-auto"
                          style={{ backgroundColor: bg }}>
                          <span className={`font-bold text-sm ${isDiag ? 'text-emerald-400' : val > 0 ? 'text-red-400' : 'text-slate-700'}`}>{val}</span>
                          <span className="text-[9px] text-slate-600">{rowTotal > 0 ? ((val/rowTotal)*100).toFixed(0) : 0}%</span>
                        </div>
                      </td>
                    )
                  })}
