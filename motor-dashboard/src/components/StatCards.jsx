...
export default function StatCards({ data }) {
  ...
  const minOf = key => Math.min(...data.map(d => parseFloat(d[key]) || 0))
  const maxOf = key => Math.max(...data.map(d => parseFloat(d[key]) || 0))
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
      <Card icon={IconDatabase}      label="Total Records"    value={total.toLocaleString()}
        ...
        accent="#3b82f6" pct={(healthy/total)*100} delta="Live" />
