...
const MODEL_KEYS = [
  { key: 'randomForest', label: 'Random Forest', color: '#3b82f6' },
...
function shortName(name) {
  return name
    .replace('CURRENT (A) ', 'I_')
    .replace('ROTO (RPM) ', 'RPM_')
    .replace('Frequency Center', 'FreqCtr')
    .replace('Spectrum Area', 'SpecArea')
    .replace('peak_to_peak', 'p2p')
    .replace('crest_factor', 'crest')
    .replace('kurtosis', 'kurt')
    .replace('skew', 'skew')
    .replace('Amp @ ', '@')
}
        ...
        <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 10, left: 60, bottom: 0 }}>
          ...
          <Bar dataKey="value" radius={[0, 4, 4, 0]}>
            {chartData.map((entry, i) => {
              ...
              const color = MODEL_KEYS.find(m => m.key === activeModel)?.color || '#3b82f6'
              return <Cell key={i} fill={color} fillOpacity={0.3 + intensity * 0.7} />
            })}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
